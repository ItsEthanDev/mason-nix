...
// One CRT texel per font pixel. iCellSize.xy is the true terminal cell size in
// screen pixels (from Ghostty's renderer); it updates with font-size and zoom.
vec2 ghosttyPixelScale() {
  vec2 cell = iCellSize.xy;
  if (cell.x <= 0.0 || cell.y <= 0.0) {
    return vec2(1.0);
  }
  return FONT_NATIVE_PIXELS / cell;
}
// "Tube" warp
#define CRTS_WARP 1
